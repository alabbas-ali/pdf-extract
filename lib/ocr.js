 ...
 */
module.exports = (input_path, options, callback) => {
	...
	exec(cmd, procoptions, (err, _stdout, _stderr) => {
		if (err)
			return callback(err)
		...
		fs.readFile(text_output_path, 'utf8', (err, output) => {
			// inspect(output, 'ocr output')
			if (err)
				return callback(err)

			// cleanup after ourselves
			fs.unlink(text_output_path, (err) => {
