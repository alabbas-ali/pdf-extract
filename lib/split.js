 ...
 */
module.exports = (pdf_path, pdf_password, callback) => {

	if (!callback || typeof pdf_password === 'function') {
		callback = pdf_password
	}

	confirm_file_exists(pdf_path, (err) => {
		...
		fs.mkdir(output_dir, (err) => {
			...
			var cmd
			if (typeof pdf_password === 'string') {
				cmd = 'pdftk ' + pdf_path + ' input_pw ' + pdf_password + ' burst output ' + output_path
			}
			else {
				cmd = 'pdfseparate ' + pdf_path + ' ' + output_path
			}
			var child = exec(cmd, (err, _stdout, _stderr) => {
				if (err) {
					var output_err = {
